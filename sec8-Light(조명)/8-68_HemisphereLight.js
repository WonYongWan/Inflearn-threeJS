...
export default function example() {
  ...
  // ------------------------------------------------------------------------------------------- 학습 내용
  // HemisphereLight는 ambientLight처럼 은은하게 비춰주지만 HemisphereLight는 위치에 따라 서로 반대의 지점에서 지정한 각각의 색을 비춘다.
  const light = new THREE.HemisphereLight('pink', 'lime', 2); // 색1, 색2, 빛의 양
  light.position.y = 2;
  scene.add(light);

  const lightHelper = new THREE.HemisphereLightHelper(light); // 시각적으로 라이트의 위치를 알려줌.
  scene.add(lightHelper);
  // -------------------------------------------------------------------------------------------

  // 그림자 설정
  // HemisphereLight는 그림자가 없으므로 사용 못함.
