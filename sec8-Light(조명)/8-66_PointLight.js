...
export default function example() {
  ...
  // ------------------------------------------------------------------------------------------- 학습 내용
  // PointLight는 어떤 지점에 있는 전구 또는 가로등이라고 생각하면 됨.
  const light = new THREE.PointLight('#fff', 10, 100, 1); // 색, 빛의 양, distance(최대 거리) 기본값 100, 거리에 따라 빛이 얼마나 어두워지게 할건지(2가 적당하다고 함.)
  light.position.y = 3;
  scene.add(light);

  const lightHelper = new THREE.PointLightHelper(light); // 시각적으로 라이트의 위치를 알려줌.
