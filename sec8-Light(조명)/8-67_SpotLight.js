...
export default function example() {
  ...
  // ------------------------------------------------------------------------------------------- 학습 내용
  // SpotLight는 원뿔 모양의 라이트에서 빛을 뿜어낸다고 생각하면 됨. 어려우면 뮤지컬 무대 조명이라고 생각하면 됨.
  const light = new THREE.SpotLight('#fff', 100, 10, Math.PI / 6); // 색, 빛의 양, distance(최대 거리), 원뿔 모양의 각도(라디안 값)
  light.position.x = -5;
  light.position.y = 3;
  scene.add(light);

  const lightHelper = new THREE.SpotLightHelper(light); // 시각적으로 라이트의 위치를 알려줌.
