...
export default function example() {
  ...
  for (let i = 0; i < spherePositionArray.length; i += 3) {
    imagePanel = new ImagePanel({
      ...
      imageSrc: `../images/textures/panelImages/0${Math.ceil(Math.random() * 5)}.jpg`,
  ...
  function setShape(e) {
    ...
    for (let i = 0; i < imagePanels.length; i++) {
      ...
      gsap.to(imagePanels[i].mesh.position, {
        duration: 2, // 몇초동안 애니메이션 할건지
        x: array[i * 3], // 3을 곱해주는 이유: randomPositionArray의 이미지패널은 1개당 값을 3개씩 갖고 있기 때문에
        y: array[i * 3 + 1],
        z: array[i * 3 + 2],
      });
